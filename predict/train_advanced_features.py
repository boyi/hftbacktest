...
logger.info("="*70)
logger.info("高级微观结构特征训练")
logger.info("="*70)

logger.info("\n[1/5] 加载数据（使用3天快速验证）...")
all_ob_l1, all_ob_l50, all_trade = [], [], []

dates = ['20250927', '20250928', '20250929']  # 3天数据快速验证
...
trade_df['signed_volume'] = trade_df['volume'] * (2 * trade_df['is_buy'] - 1)  # Buy=+1, Sell=-1
...
logger.info(f"  New advanced features: {len([c for c in feature_cols if any(x in c for x in ['microprice', 'ofi', 'lob', 'fragility', 'impact', 'regime'])])}")
...
logger.info("\n" + "="*70)
logger.info(f"高级特征模型结果 - {len(df):,} 样本")
logger.info("="*70)
...
# 保存
joblib.dump(model, 'models/saved/model_advanced.pkl')
fi.to_csv('results/feature_importance_advanced.csv', index=False)

logger.info("\n✅ 训练完成！")
logger.info("  模型: models/saved/model_advanced.pkl")
